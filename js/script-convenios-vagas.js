...
function applyVagaFilters(vagas) {
  ...
  return vagas.filter((vaga) => {
    ...
    const hasPolicial = hasPolicialEscalado(vaga);
    const searchable = [
      vaga.nomeServico,
      ...
      offerStatus.note,
      policialText,
      hasPolicial ? 'policial escalado' : 'sem policial'
...
/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Define se uma vaga ainda pode ser excluída diretamente pelo convênio.
 * Depois que a data de liberação/programação passa, a exclusão direta deixa de ser permitida.
 *
 * PARÂMETROS E RETORNO:
 * @param {object} vaga - Vaga persistida localmente com data de serviço.
 * @returns {boolean} Verdadeiro quando o botão "Excluir" pode ser exibido na tabela.
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Não grava dados; lê somente a data da vaga em memória e calcula a data de liberação automática.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: quando houver backend, validar esta regra no servidor e registrar solicitações de exclusão
 * excepcional em fluxo próprio de autorização do GSI.
 */
function canDeleteVagaDirectly(vaga) {
  const releaseAt = getAutomaticOfferDeadline(vaga.dataServico);
  if (!releaseAt) return true;

  const today = new Date();
  today.setHours(0, 0, 0, 0);
  return today < releaseAt;
}
...
function getVagaOfferStatus(vaga) {
  ...
  const authorizedAt = vaga.permissaoGsiAutorizadaAt || vaga.permissaoGsiDecididaAt || '';
  const authorizedDateObject = authorizedAt ? new Date(authorizedAt) : null;
  const authorizedDate = authorizedDateObject && !Number.isNaN(authorizedDateObject.getTime()) ? toDateInputValue(authorizedDateObject) : '';
  const authorizedDateLabel = authorizedDate ? formatDate(authorizedDate) : '';

  if (hasPolicialEscalado(vaga)) {
    return {
      offered: true,
      late: false,
      requested: false,
      ...
      className: 'assigned',
      filterValue: 'policial-escalado',
      label: 'POLICIAL ESCALADO',
      note: '',
      releaseDateLabel: authorizedDateLabel || releaseDateLabel,
      icon: ''
    };
  }
  ...
  if (approvedByGsi) {
    return {
      ...
      label: 'AUTORIZADA',
      note: '',
      releaseDateLabel: authorizedDateLabel,
      icon: ''
    };
  }
  ...
  if (createdAfterRelease) {
    return {
      ...
      late: true,
      requested,
      needsPermission: !requested,
      className: requested ? 'waiting-release' : 'pending',
      filterValue: requested ? 'aguardando-gsi' : 'fora-prazo',
      label: requested ? 'AGUARDAR LIBERAÇÃO' : 'SOLICITAR LIBERAÇÃO',
      note: '',
      releaseDateLabel: '',
      icon: ''
    };
...
/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Verifica se a vaga já possui policial escalado, preservando compatibilidade com o modelo
 * atual de `escalados` e com campos legados usados no protótipo.
 *
 * PARÂMETROS E RETORNO:
 * @param {object} vaga - Vaga persistida localmente.
 * @returns {boolean} Verdadeiro quando existe policial vinculado à vaga.
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Não grava dados; lê somente campos da vaga carregada de `cproeis_convenios_vagas`.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: Migrar esta verificação para uma entidade oficial de escalas no backend, com ID funcional obrigatório.
 */
function hasPolicialEscalado(vaga) {
  return Boolean(
    (Array.isArray(vaga.escalados) && vaga.escalados.length > 0) ||
    vaga.policialEscalado ||
    Number(vaga.preenchidas || 0) > 0
  );
}
...
/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Normaliza a lista de policiais escalados em uma vaga para permitir presença individual.
 *
 * PARÂMETROS E RETORNO:
 * @param {object} vaga - Vaga persistida no LocalStorage.
 * @returns {Array<object>} Lista de escalados, incluindo compatibilidade com campos antigos.
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Não grava dados; lê somente o objeto de vaga carregado de `cproeis_convenios_vagas`.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: Migrar escalas e presenças para tabela própria, com assinatura do convênio e auditoria.
 */
function getVagaEscalados(vaga) {
...
function renderVagasTable(convenio) {
  ...
  if (!vagas.length) {
    body.innerHTML = '<tr><td class="empty" colspan="10">Nenhuma vaga encontrada para os filtros selecionados.</td></tr>';
    return;
  }
  ...
  body.innerHTML = vagas.map((vaga) => {
    ...
    /*
     * DESCRIÇÃO DO BLOCO: Mantém a linha da tabela limpa, exibindo somente status e um
     * botão neutro para abrir a janela de ações da vaga.
     * PARÂMETROS E RETORNO: Usa vaga.id para montar um botão que abre o modal operacional.
     * ARMAZENAMENTO E PERSISTÊNCIA: Não grava dados; o modal aberto pelo botão concentra
     * as ações que poderão alterar o LocalStorage.
     * TODO: Em produção, controlar a disponibilidade das ações por permissões vindas da API.
     */
    const actionsButton = `<button type="button" class="open-actions-button" data-action="open-vaga-actions" data-id="${vaga.id}">Ações</button>`;
    ...
    return `
      <tr${rowClass}>
        ...
        <td class="release-date-cell">
          <!--
            DESCRIÇÃO DO BLOCO: Exibe a data de liberação automática da vaga em coluna própria,
            usando a mesma regra operacional do status de oferta.
            PARÂMETROS E RETORNO: Não recebe parâmetros no HTML; renderiza o texto calculado
            em offerStatus.releaseDateLabel para cada vaga.
            ARMAZENAMENTO E PERSISTÊNCIA: Não grava dados; a data é derivada em memória a partir
            da data de serviço persistida no LocalStorage.
            TODO: Em produção, exibir a data oficial retornada pelo workflow do GSI/backend.
          -->
          ${escapeHtml(offerStatus.releaseDateLabel)}
        </td>
        <td>
...
/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Abre ou fecha a janela de ações de uma vaga sem alterar os dados da tabela.
 *
 * PARÂMETROS E RETORNO:
 * @param {boolean} visible - Define se o modal ficará visível.
 * @returns {void}
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Não lê nem grava LocalStorage; altera somente classes e atributos do DOM.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: substituir o controle manual por componente modal compartilhado com foco preso e acessibilidade completa.
 */
function toggleVagaActionModal(visible) {
  const modal = document.getElementById('vaga-action-modal');
  if (!modal) return;
...
/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Renderiza as opções operacionais disponíveis para uma vaga selecionada pelo convênio.
 *
 * PARÂMETROS E RETORNO:
 * @param {object} vaga - Vaga persistida localmente.
 * @returns {void}
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Lê dados da vaga em memória, que vieram da chave `cproeis_convenios_vagas` do LocalStorage.
 * Não grava dados; os botões renderizados acionam listeners que persistem as mudanças.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: incluir campo de justificativa obrigatória e validação de perfil antes de enviar pedidos ao GSI.
 */
function renderVagaActionModal(vaga) {
  const title = document.getElementById('vaga-action-title');
  const meta = document.getElementById('vaga-action-meta');
  const details = document.getElementById('vaga-action-details');
  const statusTarget = document.getElementById('vaga-action-status');
  const list = document.getElementById('vaga-action-list');
  if (!title || !meta || !details || !statusTarget || !list) return;

  const offerStatus = getVagaOfferStatus(vaga);
  title.textContent = vaga.nomeServico || 'Vaga sem nome';
  meta.textContent = `${formatDate(vaga.dataServico)} | ${gruposClasse[vaga.classe] || vaga.classe} | ${tiposServico[vaga.tipoServico] || vaga.tipoServico}`;
  /*
   * DESCRIÇÃO DO BLOCO: Preenche os dados principais da vaga dentro do modal de ações,
   * permitindo que o usuário confira horário, liberação e escala antes de enviar pedidos ao GSI.
   * PARÂMETROS E RETORNO: Usa os campos da vaga e o status calculado; renderiza HTML informativo.
   * ARMAZENAMENTO E PERSISTÊNCIA: Não grava dados; lê somente o objeto da vaga vindo do LocalStorage.
   * TODO: Em produção, adicionar unidade solicitante, motivo de alteração e usuário responsável pela ação.
   */
  details.innerHTML = `
    <div class="vaga-action-detail">
      <span>Início</span>
      <strong>${escapeHtml(vaga.horaInicio || '-')}</strong>
    </div>
    <div class="vaga-action-detail">
      <span>Término</span>
      <strong>${escapeHtml(vaga.horaFim || '-')}</strong>
    </div>
    <div class="vaga-action-detail">
      <span>Liberação</span>
      <strong>${escapeHtml(offerStatus.releaseDateLabel || '-')}</strong>
    </div>
    <div class="vaga-action-detail">
      <span>Policial</span>
      <strong>${escapeHtml(formatPolicialEscalado(vaga) || '-')}</strong>
    </div>
  `;
  ...
  const items = [];
  const escalados = getVagaEscalados(vaga);

  if (escalados.length) {
    items.push(`
      <div class="vaga-action-item presence-action-block">
        <div>
          <strong>Presença do policial</strong>
          <span>Confirma a execução do serviço para liberar o registro à FOPAG.</span>
        </div>
        <div class="presence-list">
          ${escalados.map((escala, index) => {
            const identity = escala.policialId || escala.idFuncional || escala.nome || String(index);
            const confirmed = Boolean(escala.presencaConfirmadaAt);
            return `
              <div class="presence-row">
                <span>
                  <strong>${escapeHtml(escala.nome || 'Policial escalado')}</strong>
                  <small>${confirmed ? `Presença em ${formatDate(String(escala.presencaConfirmadaAt).slice(0, 10))}` : 'Aguardando presença'}</small>
                </span>
                ${confirmed
                  ? '<span class="action-status presence-confirmed">Presente</span>'
                  : `<button type="button" class="presence-action" data-action="confirm-presenca" data-id="${vaga.id}" data-policial-id="${escapeHtml(identity)}">Dar presença</button>`}
              </div>
            `;
          }).join('')}
        </div>
  ...
  if (offerStatus.needsPermission) {
    items.push(`
      <div class="vaga-action-item">
        <div>
          <strong>Solicitar liberação ao GSI</strong>
          <span>Envia a vaga para avaliação porque ela foi criada fora da janela de liberação.</span>
        </div>
        <button type="button" class="warning-action" data-action="request-gsi-permission" data-id="${vaga.id}">Enviar</button>
      </div>
    `);
  } else if (vaga.permissaoGsiStatus === 'solicitada') {
    items.push(`
      <div class="vaga-action-item">
        <div>
          <strong>Liberação solicitada</strong>
          <span>O pedido de liberação já está aguardando avaliação do GSI.</span>
        </div>
        <span class="action-status release-pending">Aguardando GSI</span>
      </div>
    `);
  }

  if (canDeleteVagaDirectly(vaga)) {
    items.push(`
      <div class="vaga-action-item">
        <div>
          <strong>Excluir vaga</strong>
          <span>Remove a vaga enquanto ela ainda está dentro da janela permitida para exclusão direta.</span>
        </div>
        <button type="button" class="danger" data-action="delete-vaga" data-id="${vaga.id}">Excluir</button>
      </div>
    `);
  } else if (vaga.exclusaoGsiStatus === 'solicitada') {
    items.push(`
      <div class="vaga-action-item">
        <div>
          <strong>Exclusão solicitada</strong>
          <span>O pedido de exclusão já foi enviado e está aguardando avaliação do GSI.</span>
        </div>
        <span class="action-status deletion-pending">Aguardando GSI</span>
      </div>
    `);
  } else {
    items.push(`
      <div class="vaga-action-item">
        <div>
          <strong>Solicitar exclusão ao GSI</strong>
          <span>A exclusão direta não está mais disponível; envie o pedido para avaliação.</span>
        </div>
        <button type="button" class="delete-request-action" data-action="request-gsi-delete-permission" data-id="${vaga.id}">Enviar</button>
      </div>
    `);
  }

  list.innerHTML = items.join('');
  toggleVagaActionModal(true);
}

/**
 * DESCRIÇÃO DA FUNÇÃO:
 * Liga eventos de fechamento da janela de ações por botão, fundo e tecla Escape.
 *
 * PARÂMETROS E RETORNO:
 * @returns {void}
 *
 * ARMAZENAMENTO E PERSISTÊNCIA:
 * Não lê nem grava dados; apenas registra listeners no DOM.
 *
 * NOTAS DE EXPANSÃO:
 * TODO: integrar o modal ao gerenciador global de componentes quando houver framework frontend.
 */
function bindVagaActionModalEvents() {
  const modal = document.getElementById('vaga-action-modal');
  if (!modal || modal.dataset.bound === 'true') return;

  modal.dataset.bound = 'true';
  document.getElementById('close-vaga-action-modal')?.addEventListener('click', () => toggleVagaActionModal(false));

  modal.addEventListener('click', (event) => {
    if (event.target === modal) toggleVagaActionModal(false);
  });

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !modal.classList.contains('is-hidden')) {
      toggleVagaActionModal(false);
    }
  });
}
...
function bindOperationalEvents(convenio) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (button.dataset.action === 'open-vaga-actions') {
      renderVagaActionModal(vaga);
      return;
    }
    ...
    if (button.dataset.action === 'request-gsi-delete-permission') {
      /*
       * DESCRIÇÃO DO BLOCO: Registra a solicitação excepcional de exclusão para análise do GSI.
       * PARÂMETROS E RETORNO: Usa o id da vaga presente no botão clicado; não retorna valor.
       * ARMAZENAMENTO E PERSISTÊNCIA: Atualiza a lista cproeis_convenios_vagas no LocalStorage,
       * adicionando exclusaoGsiStatus e exclusaoGsiSolicitadaAt à vaga solicitada.
       * TODO: Em produção, exigir justificativa, autenticar o usuário solicitante e enviar a
       * solicitação para um endpoint com trilha de auditoria.
       */
      const updated = getVagas().map((item) => item.id === vaga.id
        ? {
            ...item,
            exclusaoGsiStatus: 'solicitada',
            exclusaoGsiSolicitadaAt: new Date().toISOString()
          }
        : item);
    ...
    if (button.dataset.action === 'confirm-presenca') {
      /*
       * DESCRIÇÃO DO BLOCO: Confirma a presença de um policial escalado pelo convênio.
       * PARÂMETROS E RETORNO: Usa o id da vaga e a identidade do policial presentes no botão.
       * ARMAZENAMENTO E PERSISTÊNCIA: Atualiza cproeis_convenios_vagas no LocalStorage,
       * gravando presencaConfirmadaAt em escalados e dados de presença na vaga.
       * TODO: Em produção, exigir autenticação do responsável, georreferência opcional e auditoria.
       */
      const now = new Date().toISOString();
      const targetPolicial = button.dataset.policialId || '';
      const updated = getVagas().map((item) => {
        if (item.id !== vaga.id) return item;
        const escalados = getVagaEscalados(item).map((escala, index) => {
          const identity = escala.policialId || escala.idFuncional || escala.nome || String(index);
          if (String(identity) !== String(targetPolicial)) return escala;
          return {
            ...escala,
            presencaConfirmadaAt: escala.presencaConfirmadaAt || now,
            presencaConfirmadaPorConvenio: convenio.id
          };
        });
        ...
        return {
          ...item,
          escalados,
          presencasConfirmadas: escalados.filter((escala) => escala.presencaConfirmadaAt).length,
          ultimaPresencaConfirmadaAt: now
        };
      });

      saveList(STORAGE_VAGAS, updated);
      const refreshed = updated.find((item) => item.id === vaga.id);
      if (refreshed) renderVagaActionModal(refreshed);
      renderOperationalView(convenio);
    }
  });
}
...
*/
function inicializarMenuHamburgerConveniosVagas() {
...
inicializarMenuHamburgerConveniosVagas();
